...
Configurando el protocolo de recolección (Workflow en Samplers) y unos parámetros más, con Samplers se puede generar una app lista para ejecutarse en un dispositivo móvil Android. Esta app generada sirve para tomar muestras siguiendo el protocolo de recolección especificado, y las almacena en el dispositivo móvil hasta que puedan ser enviadas a un servidor web previamente configurado.

La app generada contiene una Activity principal que muestra un texto de bienvenida (configurable) y un botón para tomar una muestra. Al presionar dicho botón se llama a la Activity encargada de tomar las muestras siguiendo el protocolo de recolección configurado. 

Interactuando con el usuario final de la app, el científico ciudadano, se lo va guiando 
...
\item Mostrar al científico ciudadano las características del mosquito Aedes Aegypti para que las pueda comparar con el mosquito encontrado (para no enviar fotos innecesarias)

\item Pedir al científico ciudadano que tome una foto desde arriba del mosquito

\item Pedir al científico ciudadano que tome una foto de costado del mosquito

\item Pedir al científico ciudadano que tome la posición del GPS

\end{enumerate}

El archivo de configuración para Samplers quedaría de la siguiente manera: 

[foto o codigo del archivo de configuración con el workflow]

Con este archivo de configuración, Samplers generará una app lista para ejecutar en un dispositivo móvil Android que se verá como se muestra en la siguiente imagen:

[capturas de pantalla de la aplicación generada]

Cómo se configura el archivo de configuración de Samplers y las opciones disponibles para el mismo se explican en la siguiente sección.

Como Samplers genera el código fuente de la app, esto permite que un usuario con conocimientos de programación pueda modificarlo y personalizar la app a su gusto, así como también agregarle otras funcionalidades a la app generada.
...
%**************************
% Encarar de forma que lo primero (mas facil) es armar el arvhivo de cofiguracion y generar la app, y despues (mas avanzado) es usar las clases (que se pueden usar en una app existente)
...
Como se configura el Workflow

Los Steps disponibles son:
...
Los demás parámetros de la app.


\subsection{Usando las clases}
...
TakeSampleActivity es una Activity para tomar la muestra/ "encargada de tomar la muestra".
