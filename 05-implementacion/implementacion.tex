...
\begin{itemize}
\item Extendiendo las clases: **FALTA DESARROLLAR**
\item Usando el generador de clases de Gradle: que básicamente lo que hace es extender e instanciar las clases necesarias para generar la aplicación en base al archivo de configuración JSON pasado. **FALTA DESARROLLAR**
\end{itemize}
...
El Step tiene asociado un StepFragment (la vista y controlador) que es el encargado de ejecutar el Step para generar un StepResult. El Step básicamente tiene los parámetros o información para que pueda ser ejecutado por el StepFragment, pero no tiene mucho comportamiento.

El Step conoce cuál es el siguiente Step (su id) a ejecutar y a veces éste depende del StepResult generado, como es el caso de SelectOneStep en el que el siguiente paso depende de la opción seleccionada, y con esto se pueden crear bifurcaciones en el Workflow.
...
Si bien estos son los Steps que provee Samplers de manera predeterminada, un usuario programado podría definir y agregar sus propios Steps, junto con sus StepFragments y StepResults como se explica en la sección \ref{sec:definir_steps}.

\subsubsection{StepFragment: la vista y controlador del Step}
Es la encargada de ejecutar un Step y generar un StepResult con la interacción del usuario final (el científico ciudadano). 

Es una clase abstracta, ya que su comportamiento depende de la implementación de cada tipo de StepFragment que se defina. 

Se definió como una clase abstracta y no como una interfaz por la necesidad de que heredara de Fragment.

**FALTA DESARROLLAR**

\subsubsection{StepResult: el Resultado de la ejecución de un Paso}
Los StepResult son el resultado obtenido de ejecutar un Step. Cada ejecución de un mismo Step, puede generar un StepResult diferente.

Un StepResult tiene el Id del Step en base al cual se generó. 

También puede tener asociado un archivo multimedia, por ejemplo en los casos de PhotoStep y SoundRecordStep que guardan una foto y un archivo de sonido respectivamente.

**FALTA DESARROLLAR**

se pueden agregar steps propios, solo tienen que implementar la interfaz.
...
También guarda fecha y hora de inicio y finalización. Esto es útil para sacar una estadística de cuanto tarda un usuario en recolectar una muestra y analizar optimizaciones para la aplicación final.
...
La clase TakeSampleActivity es la encargada de ejecutar el Worflow para tomar la muestra. Es una activity que recibe un objeto Workflow como parámetro y y va iterando sobre los Steps del mismo. A cada Step le pide su StepFragment...
...
Las muestras se envían al servidor web previamente configurado, mediante un mensaje HTTP POST. Para ello se usó la librería OkHttp\cite{okhttp}, distribuida por Square Inc. bajo licencia Apache 2.0, que resuelve de manera sencilla y muy eficiente el envío de mensajes HTTP en Android.

Por cada muestra se envía el objeto Sample en formato JSON junto con los archivos multimedia, todo comprimido en un solo archivo ZIP.

Para comprimir se usaron las librerías estándares de Java (java.util.zip) que proporciona clases para leer y escribir archivos ZIP y GZIP estándares.
